...
restriccioEventDia :- %un event es produeixi com a maxim un dia
	event(E),
...
restriccioDiesEvents :- %maxim n events per dia
	day(D),
	findall(ed(E,D),event(E),L),
	maxEventsPerDay(M),
	atMost(M,L),
	fail.
restriccioDiesEvents.

restriccioSempreModerador :- %un event tingui com a maxim un moderador
	event(E),
	findall(em(E,M),moderator(M),L),
	exactly(1,L),
	fail.
restriccioSempreModerador.

restriccioDiesModerador :- %tope de dies per moderador
	moderator(M),
	findall(md(M,D),day(D),L),
	maxDaysPerModerator(K),
	atMost(K,L),
	fail.
restriccioDiesModerador.

restriccioModeratorDia :- %maxim un moderador per event per dia
	
	moderator(M),
	findall(md(M,D),day(D),L),
	exactly(1,L),
	fail.
restriccioModeratorDia.


restriccioJuntarClausules :- %juntem clausules
	day(D),
	event(E),
	moderator(M),
	...
	writeClause([-ed(E,D),-em(E,M), md(M,D)]),
	fail.
restriccioJuntarClausules.
