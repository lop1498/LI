...
minim1trajecteperciutat :-
	
	trip(C1-C2),
	findall(bdcc(B,D,C1,C2),(bus(B),notLastDay(D)),L),
	atLeast(1,L),
	fail.
minim1trajecteperciutat.
...
restriccioVariables :-
	...
	bus(B),
	writeClause([-bdcc(B,D1,C1,C2),bdc(B,D1,C1)]),
	writeClause([-bdcc(B,D1,C1,C2),bdc(B,D2,C2)]),
...
atLeast1Bus :- %com a minim un bus per ciutat
	city(C),
	day(D),
	findall(bdc(B,D,C),bus(B),L),
	exactly(1,L),
	fail.
atLeast1Bus.
...
displaySol(M) :- nl, city(C),write('La ciutat '), write(C),write(' te: '), findall(B-D,(member(bdc(B,D,C),M)),L), write(L), nl, fail.
displaySol(M):-
    nl,
